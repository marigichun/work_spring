...
public class UserDaoTest {
	...
	// 픽스쳐(ficture): 테스트를 수행하는데 필요한 정보나 -->> 객체 20~23줄의 필드변수
	// setUp() 메소드에서 만드는 객체를 테스트 메소드에서 사용할 수 있도록 필드에 선언 
	private UserDao dao;
	private User user1;
	private User user2;
	private User user3;
	...
	public void setUp() {
		...
		// 생성된 캔텍스트 객체와 UserDaoTest 객체의 주소값을 출력해보자.
	...
	public void addAndGet() throws SQLException { // JUnit 테스트 메소드는 반드시 public으로 선언
		// 전체 삭제
		dao.deleteAll();//테스트의 일부분이라 빼지 않아도 됨
	 ...
	 * 
	 * 		단계				내용						코드
	 * 조건	어떤 조겉을 가지고	가져올 사용자 정보가			dao.deleteAll()
	 * 						존재하지 않는 경우에 			assertThat(dao.getCount(), is());
	 * 
	 * 행위 	무엇을 할때			존재하지 않는 id로 get()		dao.get("unknown_id");
	 * 결과	어떤 결과가 나온다.	을 실행하면 특별한 			@Test(expected=
	 * 						예외가 던져진다.				EmptyResultDataAccessException.class)
	 */
	
	// 위와 같이 작성하면 기능 설계(위의 단계, 내용 부분), 코드 구현, 테스트 세 가지 작업이 동시에 끝!!
	
		/*
		 * 테스트 주도 개발(TDD: Test Driven Develoment)
		 * TDD의 기본 원칙: 실패한 테스트를 성공시키기 위한 목적이 아닌 코드는 만들지 않는다.
		 * TDD는 체스트를 먼저 만들고 그 테스트가 송공하도록 하는 코드만 만드는 식으로
		 * 진행하기 때문에 테스트를 빼먹지 않고 꼼꼼하게 만들어 낼 수 있다.
		 */
	
	/*
	 * JUnit 하나의 테스트 클래스를 가져와 테스트를 수행하는 방식 (7단계)
	 * 
	 * 1. 테스트 클래스에서 @Test가 붙은 public 이고 void 형이며 파라미터가 없는 
	 *    테스트 메소드를 모두 찾는다.
	 * 2. 테스트 클래스의 객체를 하나 만든다.(객체를 매번 새로 만드는 이유는 독립적으로 
	 *    실행됨을 확실히 보장해주기 위해)
	 * 3. @Before가 붙은 메소드가 있으면 실해된다.
	 * 4. @Test가 붙은 메소드를 하나 호출하고 테스트 결과를 저장해둔다.
	 * 5. @After가 붙은 메소드가 있으면 실행한다.
	 * 6. 나머지 테스트 메소드에 대해 2~5 반복한다.
